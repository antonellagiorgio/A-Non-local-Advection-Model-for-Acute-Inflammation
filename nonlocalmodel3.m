...
alpha = alphaT_nonlocal +0.1;
q6 = @(k) (D1*D2).*k.^6;
q4 = @(k) (((k1*k4 + k2*k3)*D1*D2)/(k2*(k3+k4)) + k2*D2 + k4*D1).*k.^4;
q2 = @(k) ( (k4*(k1-k2)*k2*D2)/(k2*(k3+k4))+ (k4*(k1*k4+k2*k3)*D1)/(k2*(k3+k4)) + k2*k4).*k.^2;
q0 = (k4*(k1*k4 + k2*k3)*(k1-k2))/(k1*(k3+k4));
alphaterm = @(k) (D2*k4*(k1-k2))/(k3+k4).*k.^4 + (k4^2*(k1-k2)*(k1*k4 + k2*k3))/(k1*(k3+k4)^2).*k.^2;
Qk = @(k) q6(k) + q4(k) + q2(k) + q0 - alpha.*alphaterm(k).*(sin(k.*(R))./(k.*(R)));
...
%% FTCS
[m_hist, p_hist, a_hist, Ti_hist] = nonlocalFE3(m_curr,p_curr,a_curr,C_curr,kernel,par,dx,dt,T,save_interval);
